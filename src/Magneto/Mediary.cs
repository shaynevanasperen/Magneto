...
public class Mediary(IServiceProvider? serviceProvider = null, IDecorator? decorator = null) : IMediary
{
	static readonly ConcurrentDictionary<Type, object> NullCacheStores = new();
	...
	/// </summary>
	protected IDecorator Decorator { get; } = decorator ?? serviceProvider?.GetService<IDecorator>() ?? NullDecorator.Instance;
	...
	{
		ArgumentNullException.ThrowIfNull(executedQuery);
